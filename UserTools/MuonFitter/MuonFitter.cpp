...
bool MuonFitter::Initialise(std::string configfile, DataModel &data){
  ...
  if (!isData)
  {
    ...
    if (!truetrack_file)
    {
      if(verbosity>1) std::cout << " [debug] true_track_len.txt does not exist! Creating now..." << std::endl;
...
//    std::string tmp_str = mcFile.erase(0, mcFile.find(delim) + delim.length());
    ...
    partnumber = std::stoi(tmp_str.substr(tmp_str.rfind(delim) + delim.length()));
  ...
  if (reco_mode)
  {
    ...
    if (it == m_tank_track_fits.end()) {
      std::cout << "MuonFitter skipping" << std::endl;
      m_data->Stores["RecoEvent"]->Set("JulieRecoFlag",SimpleRecoFlag);
      m_data->Stores["RecoEvent"]->Set("JulieRecoEnergy",SimpleRecoEnergy);
      m_data->Stores["RecoEvent"]->Set("JulieRecoVtx",SimpleRecoVtx);
      m_data->Stores["RecoEvent"]->Set("JulieRecoStopVtx",SimpleRecoStopVtx);
      m_data->Stores["RecoEvent"]->Set("JulieRecoCosTheta",SimpleRecoCosTheta);
      m_data->Stores["RecoEvent"]->Set("JulieRecoPt",SimpleRecoPt);
      m_data->Stores["RecoEvent"]->Set("JulieRecoFV",SimpleRecoFV);
      m_data->Stores["RecoEvent"]->Set("JulieRecoMrdEnergyLoss",SimpleRecoMrdEnergyLoss);
      m_data->Stores["RecoEvent"]->Set("JulieRecoTrackLengthInMRD",SimpleRecoTrackLengthInMRD);
      m_data->Stores["RecoEvent"]->Set("JulieRecoMRDStart",SimpleRecoMRDStart);
      m_data->Stores["RecoEvent"]->Set("JulieRecoMRDStop",SimpleRecoMRDStop);
      m_data->Stores["RecoEvent"]->Set("DownstreamFV",DownstreamFV);
      m_data->Stores["RecoEvent"]->Set("FullCylFV",FullCylFV);
      m_data->Stores["RecoEvent"]->Set("JulieRecoNeutrinoEnergy",SimpleRecoNeutrinoEnergy);
      m_data->Stores["RecoEvent"]->Set("JulieRecoQ2",SimpleRecoQ2);
      return true;
    }
  }
  ...
  if (!isData)
  {
    ...
    if(verbosity>1) std::cout << " [debug] n_rings (my counter): " << n_rings << std::endl;
  ...
  for(int track_i = 0; track_i < numTracksInEv; track_i++)
  {
    ...
    if(verbosity>1) std::cout << "  [debug] mrdStartVertex: " << mrdStartVertex.X() << "," << mrdStartVertex.Y() << "," << mrdStartVertex.Z() << std::endl;
    if(verbosity>1) std::cout << "  [debug] mrdStopVertex: " << mrdStopVertex.X() << "," << mrdStopVertex.Y() << "," << mrdStopVertex.Z() << std::endl;
    if(verbosity>1) std::cout << "  [debug] tankExitPoint: " << tankExitPoint.X() << "," << tankExitPoint.Y() << "," << tankExitPoint.Z() << std::endl;
    if(verbosity>1) std::cout << "  [debug] mrdEntryPoint: " << mrdEntryPoint.X() << "," << mrdEntryPoint.Y() << "," << mrdEntryPoint.Z() << std::endl;
    if(verbosity>1) std::cout << "  [debug] mrdStartTime: " << mrdStartTime << std::endl;
    if(verbosity>1) std::cout << "  [debug] trackAngleRad: " << trackAngleRad << std::endl;
    if(verbosity>1) std::cout << "  [debug] penetrationDepth: " << penetrationDepth << std::endl;
    if(verbosity>1) std::cout << "  [debug] reco_mrd_track (ANNIE tools): " << reco_mrd_track << std::endl;
    if(verbosity>1) std::cout << "  [debug] mrdEnergyLoss: " << mrdEnergyLoss << std::endl;
    if(verbosity>1) std::cout << "  [debug] isMrdStopped: " << isMrdStopped << std::endl;
    if(verbosity>1) std::cout << "  [debug] isMrdPenetrating: " << isMrdPenetrating << std::endl;
    if(verbosity>1) std::cout << "  [debug] isMrdSideExit: " << isMrdSideExit << std::endl;
    if(verbosity>1) std::cout << "  [debug] numLayersHit: " << numLayersHit << std::endl;
    if(verbosity>1) std::cout << "  [debug] LayersHit.size(): " << LayersHit.size() << std::endl;
    if(verbosity>1) std::cout << "  [debug] MrdPMTsHit.size(): " << MrdPMTsHit.size() << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] PCA angle (rad): " << pcaAngleRad << std::endl;
  if(verbosity>1) std::cout << " [debug] PCA angle (deg): " << pcaAngleDeg << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] PCA nlyrs_mrd_track: " << pca_mrd_track << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] FINAL conn_dots_mrd_track: " << conn_dots_mrd_track << std::endl;
  if(verbosity>1) std::cout << " [debug] FINAL nlyrs_mrd_track: " << nlyrs_mrd_track << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] mrdTrackDir, trackAngleRad: " << mrdTrackDir.Angle(TVector3(0,0,1)) << "," << trackAngleRad << std::endl;
...
/*      if (pow(v.X(),2) + pow(v.Y(),2) + pow(v.Z(),2) <= pow(tank_radius*100.-30.,2))
      ...
      if((v.Y() > -150.)&&(v.Y() < 150.)&&(120. > std::sqrt(v.X()*v.X() + v.Z()*v.Z()))){
        inSFV = true;
      }
    }
    c++;
...
/*  for (int i = 1; i < vtxCandidates.size(); ++i)
  ...
  if(verbosity>1) std::cout << " [debug] cluster_size (num clusters in event): " << cluster_size << std::endl;
  ...
  if (isData)
  {
    for (std::pair<double, std::vector<Hit>>&& apair : *m_all_clusters)
    {
      ...
      if(verbosity>1) std::cout << " [debug] temp_time [ns]: " << temp_time << std::endl;
  ...
  else  //is MC
  {
    for (std::pair<double, std::vector<MCHit>>&& apair : *m_all_clusters_MC)
    {
      ...
      if (temp_charge > main_cluster_charge)
      {
        ...
        if(verbosity>1) std::cout << " [debug] earliest_hittime: " << earliest_hittime << std::endl;
      }
    }
  }
  ...
  if (found_muon)
  {
    ...
    for (unsigned long detkey = 332; detkey < 464; ++detkey)
    {
      ...
      if(verbosity>1) std::cout << " [debug] all hit times for detkey " << detkey << ": ";
      ...
      if(verbosity>1) std::cout << " [debug] hitXYZ: " << hitX << "," << hitY << "," << hitZ << std::endl;
      if(verbosity>1) std::cout << " [debug] tankExitXYZ: " << tankExit.X() << "," << tankExit.Y() << "," << tankExit.Z() << std::endl;
      if(verbosity>1) std::cout << " [debug] vec_Ri: " << vec_Ri.X() << "," << vec_Ri.Y() << "," << vec_Ri.Z() << std::endl;
      if(verbosity>1) std::cout << " [debug] vec_Ri direction: " << vec_Ri.Unit().X() << "," << vec_Ri.Unit().Y() << "," << vec_Ri.Unit().Z() << std::endl;
      ...
      //-- QA: Check that vector was calculated correctly
      if(verbosity>1) std::cout << " [debug] vec_ai: " << vec_ai.X() << "," << vec_ai.Y() << "," << vec_ai.Z() << std::endl;
      if(verbosity>1) std::cout << " [debug] ai: " << ai << std::endl;
      if(verbosity>1) std::cout << " [debug] vec_bi: " << vec_bi.X() << "," << vec_bi.Y() << "," << vec_bi.Z() << std::endl;
      if(verbosity>1) std::cout << " [debug] bi: " << bi << std::endl;
    ...
    for (auto const &pair: m_fpmt_ai)
    {
      ...
      {
        if(verbosity>1) std::cout << " [debug] avg_eta (before dividing j, so right now it is total eta): " << avg_eta << std::endl;
        running_avg = avg_eta / (j+1);
      }

      if(verbosity>1) std::cout << " [debug] ai, running_avg: " << pair.first << ", " << running_avg << std::endl;
    ...
    if(verbosity>1) std::cout << " [debug] avg eta: " << avg_eta << ", j: " << j << std::endl;

    //-- Keep track of avg eta to the left and right of the trueTrackLegnthInWater:
    if (!isData)
    {
      if(verbosity>1) std::cout << " [debug] num pmts (left, right): " << num_left_eta << ", " << num_right_eta << std::endl;
      if (num_left_eta != 0) left_avg_eta /= num_left_eta;
      if (num_right_eta != 0) right_avg_eta /= num_right_eta;
      if(verbosity>1) std::cout << " [debug] left avg: " << left_avg_eta << ", right avg: " << right_avg_eta << std::endl;
    }
  ...
  if (reco_mode)
  {
    ...
    {
      if(verbosity>1) std::cout << " [debug] angle more than 5 degrees! Event: p" << partnumber << "_";
      if (isData) std::cout << evnum;
      else std::cout << mcevnum;
      std::cout << std::endl;
    }
...
*/
      ...
      if(verbosity>1) std::cout << " [debug] tank_mrd_dist [cm]: " << tank_mrd_dist << std::endl;
      if(verbosity>1) std::cout << " [debug] outtank_edep: " << outtank_edep << std::endl;
      ...
      if(verbosity>1) std::cout << " [debug] mrd_edep: " << mrd_edep << std::endl;  //check which MRD energy is used

      if (use_nlyrs)
      {
        if(verbosity>1) std::cout << " [debug] Adjust MRD track length based on initial mrd_edep" << std::endl;
        if (mrd_edep >= 400.)
        {
        ...
        if(verbosity>1) std::cout << " [debug] nlyrs upd mrd_track: " << mrd_track << std::endl;
        if(verbosity>1) std::cout << " [debug] nlyrs upd mrd_edep: " << mrd_edep << std::endl;
      }
      ...
      bool eres_ok = false;
      int n_tries = 0;
      if(verbosity>1) std::cout << " [debug] initial tank_dEdx: " << tank_dEdx << std::endl;
...
*/
        ...
        if(verbosity>1) std::cout << " [debug] Initial d_tanktrack: " << d_tanktrack << std::endl;

        while (d_tanktrack > 0)
        {
          if (d_tanktrack < dx)
          {
            if(verbosity>1) std::cout << " [debug] remaining d_tanktrack: " << d_tanktrack << std::endl;
            ...
            if(verbosity>1) std::cout << " [debug] input_Emu: " << input_Emu << std::endl; 
            if(verbosity>1) std::cout << " [debug] sum_tank_Emu: " << sum_tank_Emu << std::endl; 
            if(verbosity>1) std::cout << " [debug] d_tanktrack: " << d_tanktrack << std::endl;  //should be 0
            if(verbosity>1) std::cout << " [debug] final dEdx: " << deltaE/dx << std::endl; 
          }
          else
          {
            ...
            if(verbosity>1) std::cout << " [debug] input_Emu: " << input_Emu << std::endl; 
            if(verbosity>1) std::cout << " [debug] sum_tank_Emu: " << sum_tank_Emu << std::endl; 
            if(verbosity>1) std::cout << " [debug] d_tanktrack: " << d_tanktrack << std::endl;
            if(verbosity>1) std::cout << " [debug] dEdx: " << deltaE/dx << std::endl; 
          }
        }
        if(verbosity>1) std::cout << " [debug] input_Emu (for MRD): " << input_Emu << "," << reco_mu_e-sum_tank_Emu << std::endl; //should be equal
        if(verbosity>1) std::cout << " [debug] Initial reconstructed Emu: " << reco_mu_e << std::endl;
        //-- Update reco muon energy with the new tank energy (same initial MRD energy deposition)
        reco_mu_e = sum_tank_Emu + mrd_edep;
        if(verbosity>1) std::cout << " [debug] New reconstructed Emu (updated tank energy): " << reco_mu_e << std::endl;
        ...
        if(verbosity>1) std::cout << " [debug] Initial d_mrdtrack: " << d_mrdtrack << std::endl;
        while (d_mrdtrack > 0)
        {
          ...
          if (input_Emu < 20.)
          {
            if(verbosity>1) std::cout << " [debug] input_Emu is LESS THAN 20 MeV: " << input_Emu << std::endl;
            if(verbosity>1) std::cout << " [debug] Remaining d_mrdtrack: " << d_mrdtrack << std::endl;
            ...
            if(verbosity>1) std::cout << " [debug] sum_mrd_Emu: " << sum_mrd_Emu << std::endl; 
            input_Emu -= input_Emu;
          ...
          if (d_mrdtrack < dx)
          {
            ...
            if(verbosity>1) std::cout << " [debug] input_Emu: " << input_Emu << std::endl;
            if(verbosity>1) std::cout << " [debug] sum_mrd_Emu: " << sum_mrd_Emu << std::endl; 
            if(verbosity>1) std::cout << " [debug] d_mrdtrack: " << d_mrdtrack << std::endl;
            if(verbosity>1) std::cout << " [debug] final dEdx: " << deltaE/dx << std::endl; 
          }
          else
          {
        ...
        if (use_simple_ereco) 
        {
          ...
          if(verbosity>1) std::cout << " [debug] reconstructed Emu (Simple E Reco): " << reco_mu_e << std::endl;
        }
...
/*    if (!isData)
  ...
  std::cout << "MuonFitter: Fitted vertex X: " << fitted_muon_vtx.X() << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] SimpleRecoFlag: " << SimpleRecoFlag << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] DownstreamFV (before): " << DownstreamFV << std::endl;
  if(verbosity>1) std::cout << " [debug] FullCylFV (before): " << FullCylFV << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] DownstreamFV (after): " << DownstreamFV << std::endl;
  if(verbosity>1) std::cout << " [debug] FullCylFV (after): " << FullCylFV << std::endl;
  ...
  if(verbosity>1) std::cout << " [debug] test_fv (DownstreamFV): " << test_fv << std::endl;
  m_data->Stores["RecoEvent"]->Get("FullCylFV", test_fv);
  if(verbosity>1) std::cout << " [debug] test_fv (FullCylFV): " << test_fv << std::endl;
